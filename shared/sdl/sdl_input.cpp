...
static void IN_TranslateNumpad( SDL_Keysym *keysym, keyNum_t *key )
{
...
===============
*/
static keyNum_t IN_TranslateSDLToJKKey( SDL_Keysym *keysym, qboolean down ) {
	keyNum_t key = (keyNum_t)0;

	if ( keysym->sym >= K_LOW_A && keysym->sym <= K_LOW_Z )
		key = (keyNum_t)(K_CAP_A + (keysym->sym - K_LOW_A));
	//else if ( keysym->sym >= A_LOW_AGRAVE && keysym->sym <= A_LOW_THORN && keysym->sym != A_DIVIDE )
	//	key = (keyNum_t)(A_CAP_AGRAVE + (keysym->sym - A_LOW_AGRAVE));
	...
	while( SDL_PollEvent( &e ) )
	{
		switch( e.type )
		{
			case SDL_KEYDOWN:
				...
				else if ( kg.keys[K_CTRL].down && key >= K_CAP_A && key <= K_CAP_Z )
				...
				if( lastKeyDown != K_CONSOLE )
				{
					...
					while( *c )
					{
						...
						if( utf32 != 0 )
						{
							...
							{
								Sys_QueEvent( 0, SE_KEY, K_CONSOLE, qtrue, 0, NULL );
								Sys_QueEvent( 0, SE_KEY, K_CONSOLE, qfalse, 0, NULL );
							}
							else
							{
			...
			case SDL_MOUSEBUTTONUP:
				{
					...
					switch( e.button.button )
					{
						...
						default: b = K_AUX1 + ( e.button.button - SDL_BUTTON_X2 + 1 ) % 32; break;
					}
