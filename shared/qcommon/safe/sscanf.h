...
namespace Q
{
	namespace detail
	{
		...
		*/
		...
		class ArraySpanStreambuf : public std::basic_streambuf< CharT >
		{
		public:
			ArraySpanStreambuf( const gsl::basic_string_span< const CharT >& view )
			{
		...
		*/
		...
		std::size_t sscanf_impl_stream( const gsl::cstring_span& input, const std::size_t accumulator, T& value, Tail&&... tail )
		{
			ArraySpanStreambuf< char > buf{ input };
	...
	Works very much like std::sscanf; but the input is a string_span instead of a null-terminated string, and the format is implicit in the arguments.
