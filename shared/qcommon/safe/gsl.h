...
// the default cstring_span constructor from string literals includes the terminating null; this one does not.
...
# define CSTRING_SPAN(x) vs2013hack_cstring_span_literal(x)
template< int length >
inline gsl::cstring_span vs2013hack_cstring_span_literal( const char (&str)[length] )
{
	static_assert( length > 0, "CSTRING_SPAN expects a string literal argument." );
	return{ str, str + length - 1 };
}
#else
# define CSTRING_SPAN(x) x ## _v
