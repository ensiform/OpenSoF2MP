...
static void DoSyscall(void)
{
	...
	if(vm_syscallNum < 0)
	{
		...
		*ret = savedVM->syscall((intptr_t *) data);
#endif
	}
	else
	{
...
void VM_Compile(vm_t *vm, vmHeader_t *header)
{
	...
	buf = (byte *)Z_Malloc(maxLength, TAG_VM, qfalse);
	jused = (byte *)Z_Malloc(jusedSize, TAG_VM, qfalse);
	code = (byte *)Z_Malloc(header->codeLength+32, TAG_VM, qfalse);
