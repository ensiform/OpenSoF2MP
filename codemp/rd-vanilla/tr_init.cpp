...
static void GLW_InitTextureCompression( void )
{
	...
	if ( !r_ext_compressed_textures->value )
	{
		...
		glConfigExt.textureCompression = TC_NONE;
	...
	else
	{
		...
		if ( r_ext_preferred_tc_method->integer == TC_NONE )
		{
			...
			if ( newer_tc )
			{
				...
				glConfigExt.textureCompression = TC_S3TC_DXT;
			}
			else
			{
				Com_Printf ("...no tc preference specified\n" );
				Com_Printf (".....using GL_S3_s3tc\n" );
				glConfigExt.textureCompression = TC_S3TC;
			}
		}
		else
		{
...
static void GLimp_InitExtensions( void )
{
	...
	glConfigExt.maxTextureFilterAnisotropy = 0;
	if ( GL_CheckForExtension( "EXT_texture_filter_anisotropic" ) )
	{
		qglGetFloatv( GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &glConfigExt.maxTextureFilterAnisotropy );
		...
		ri->Cvar_SetValue( "r_ext_texture_filter_anisotropic_avail", glConfigExt.maxTextureFilterAnisotropy );
		if ( r_ext_texture_filter_anisotropic->value > glConfigExt.maxTextureFilterAnisotropy )
		{
			ri->Cvar_SetValue( "r_ext_texture_filter_anisotropic_avail", glConfigExt.maxTextureFilterAnisotropy );
		}
	}
...
void GfxInfo_f( void )
{
	...
	ri->Printf( PRINT_ALL, "compressed textures: %s\n", enablestrings[glConfigExt.textureCompression != TC_NONE] );
	ri->Printf( PRINT_ALL, "compressed lightmaps: %s\n", enablestrings[(r_ext_compressed_lightmaps->integer != 0 && glConfigExt.textureCompression != TC_NONE)] );
	ri->Printf( PRINT_ALL, "texture compression method: %s\n", tc_table[glConfigExt.textureCompression] );
	ri->Printf( PRINT_ALL, "anisotropic filtering: %s  ", enablestrings[(r_ext_texture_filter_anisotropic->integer != 0) && glConfigExt.maxTextureFilterAnisotropy] );
	if (r_ext_texture_filter_anisotropic->integer != 0 && glConfigExt.maxTextureFilterAnisotropy)
	{
		...
		if (Q_isintegral(glConfigExt.maxTextureFilterAnisotropy))
			ri->Printf( PRINT_ALL, "%i)\n", (int)glConfigExt.maxTextureFilterAnisotropy);
		else
			ri->Printf( PRINT_ALL, "%f)\n", glConfigExt.maxTextureFilterAnisotropy);
	}
...
void R_Register( void )
{
	...
	r_ext_compressed_lightmaps			= ri->Cvar_Get( "r_ext_compressed_lightmaps",		"0",						CVAR_ARCHIVE|CVAR_LATCH, "" );
