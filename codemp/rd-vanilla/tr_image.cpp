...
void GL_TextureMode( const char *string ) {
	...
	if ( r_ext_texture_filter_anisotropic->value > glConfigExt.maxTextureFilterAnisotropy )
		ri->Cvar_SetValue( "r_ext_texture_filter_anisotropic", glConfigExt.maxTextureFilterAnisotropy );
	...
	while ( (glt   = R_Images_GetNextIteration()) != NULL)
	{
		if ( glt->mipmap ) {
			...
			if(glConfigExt.maxTextureFilterAnisotropy>0) {
						 ...
						 word *pUploadWidth, word *pUploadHeight, bool bRectangle = false )
{
	...
	if (mipmap)
	{
		...
		if(r_ext_texture_filter_anisotropic->integer>1 && glConfigExt.maxTextureFilterAnisotropy>0)
		{
			qglTexParameterf( GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, r_ext_texture_filter_anisotropic->value );
		}
	}
